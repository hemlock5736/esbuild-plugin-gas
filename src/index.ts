...
import type { FileExported } from "./types/exporteds.js";
import { generateGlobalAssignments } from "./utils/generateGlobalAssignments.js";
import { build, getExporteds } from "./utils/getExporteds.js";
...
const gasPlugin = (): Plugin => ({
  ...
  async setup({ initialOptions, onEnd }) {
    ...
    onEnd((result) => {
      for (const file of result.outputFiles ?? []) {
        const globalAssignments = generateGlobalAssignments(
